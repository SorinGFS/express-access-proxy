...
class Server {
    constructor(configServer) {
        ...
        this.auth.jwt.refresh = async (req) => {
            ...
            const permission = await Permissions.findOne(filter);
            if (!permission) throw createError.Forbidden();
            if (permission.expiresAt < new Date()) {
                Permissions.deleteOne(filter);
        ...
        this.auth.jwt.slideExpiration = async (req) => {
            const filter = { user: req.user };
            return await Permissions.upsertOne(filter, { expiresAt: new Date(Date.now() + req.server.auth.maxInactivitySeconds * 1000) });
        };
        this.auth.jwt.logout = async (req) => {
            const filter = { user: req.user };
            return await Permissions.deleteOne(filter);
        };
